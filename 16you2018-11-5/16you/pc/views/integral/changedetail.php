...
<body>
  <div ng-controller="LoadDatadetailCtrl">
	...
	<div class="change_mall_box">
	  	<em>
	  		<img src="<?php echo yii::$app->params['backend'];?>/media/images/product/{{loaddetail.image_url}}">
	  	</em>
				...
				<div class="myselfInfo">
					<ul class="rankDer">
						<li class="pastFontsize">
							<span class="pastRank fz_sm26">手机号码：</span>
							<input type="number"  id="tel" class="addressText fz_sm2" oninput="this.value = /^(13[0-9]|14[0-9]|15[0-9]|18[0-9])\d{8}$/.test(this.value)?this.value:this.value.substr(0,11)" required="required" name="phone" placeholder="输入11位手机号码">
						</li>
					</ul>
				</div> 
...
<script>
	...
	function changeclick(){
		...
		if(ptype==1 && !$('input[name="area"]').length){
			$('.rankDer>.pastFontsize').before('<li class="pastFontsize"><span id="pastRank" class="pastRank fz_sm26">收货地址：</span><input type="text" class="addressText fz_sm2" maxlength="100" name="area" placeholder="输入详细收货地址" onkeydown="if(event.keyCode==32) return false" required="required"></li>');
		}
	...
	$('.convertBtn').click(function(){
		...
		if(ptype==1){
			if(area.indexOf(" ") >=0){
				!$('#area_s').length && $('input[name="area"]').after("<br/><span class='error' id='area_s'>输入地址时不能含空格哦！</span>");
				return;
			}
			if(area == ""){
				!$('#area_s').length && $('input[name="area"]').after("<br/><span class='error' id='area_s'>收货地址不能为空！</span>");
				return;
			}
// 			else{
// 				!$('#area_s').length && $('input[name="area"]').after("<br/><span class='success' id='area_s'>输入合法！</span>");
// 				  return;
// 				}
		}
...
// 		else{
// 			!$('#tel_s').length && $('input[name="area"]').after("<br/><span class='success' id='area_s'>输入合法！</span>");
// 			  return;
// 			}
		$.ajax({
