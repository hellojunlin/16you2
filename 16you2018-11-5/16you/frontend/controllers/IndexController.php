...
class IndexController extends BaseController{
    ...
    public function actionGetcpdata(){
    	if(yii::$app->request->isAjax){
    		...
    		if(!yii::$app->session['plateform']){
    			$pnameobject = yii::$app->session['plateform'];
    		}else{
    			$pnameobject = Plateform::find()->where(['punid'=>$puid])->select('pname')->one();
    		}
    		$pname = isset($pnameobject->pname)?$pnameobject->pname:'';  //获取平台名称
    ...
    public function actionDetail(){
    	if(isset($_GET['id'])){
    		...
    		$gift = \Yii::$app->db->createCommand("SELECT id,gid,gift_name,game_name,content,CDKEY,state,createtime,count(gift_name) as num,game_image,number,payment,gifttype,validtime from g_gift where gid=:gid and state=0 AND type=1 AND number not in (select number from g_gift where gid=:gid and type=1 and uid=:uid) group By number order By createtime desc",[':uid'=>$uid,':gid'=>$gid])->queryAll();
    		$hotgame = array_slice(yii::$app->cache->get('hotgame'),0,8);//热门游戏//Game::find()->where(['type'=>1,'state'=>1])->limit(8)->orderBy('sort asc')->asArray()->select('id,head_img,name')->all();//热门游戏
    		if(!$hotgame){//热门游戏缓存不存在时，则去数据库重新查询
    			$this->getGame();
    			$hotgame = array_slice(yii::$app->cache->get('hotgame'),0,8);
 ...
 /**
     ...
     * 跳转提现业
     */
    public function actionToembody(){
    	if(!isset($_GET['openid'])){
    		$this->redirect('/index/index.html');
    	}
    	$openid = Helper::filtdata($_GET['openid']);
    	if(!$openid){
    		$this->redirect('/index/index.html');
    	}
    	$url = 'http://xgame.nj6080.com/user/getuserinfo.html';
    	$data['openid'] = $openid;
    	$wxinutilres = json_decode(Wxinutil::https_request($url,$data),true);
    	if($wxinutilres['errorcode']==0){//体现页面
    		return $this->renderPartial('embody',['openid'=>$openid,'price'=>$wxinutilres['msg']]);
    	}else{//跳转首页
    		$this->redirect('/index/index.html');
    	}
    }
     ...
     * 申请提现
     */
    Public function actionEmbody(){
    	if(!isset($_POST['openid'])){
    		$this->redirect('/index/index.html');
    	}
    	$openid = Helper::filtdata($_POST['openid']);
    	if(!$openid){
    		return json_encode(['errorcode'=>1001,'msg'=>'网络异常，稍后再试']);
    	}
    	$user = yii::$app->session['user'];
    	$url = 'http://xgame.nj6080.com/user/getembody.html';
    	$data['openid'] = $openid;
    	$data['uid'] = $user->id;
    	$data['ordernum'] = 'dzp'.date('YmdHis',time()).rand(1000,9999);//交易编号
    	$wxinutilres = json_decode(Wxinutil::https_request($url,$data),true);
    	if($wxinutilres['errorcode']==0){
    		$user = yii::$app->session['user'];
    		$rebate = new Rebaterecord();
    		$rebate->uid = $user->id;
    		$rebate->rebatenum = $wxinutilres['price'];
    		$rebate->state = 0;      //是否审核  0：未审核  1：已审核
    		$rebate->remark = "-1";
    		$rebate->createtime = time();
    		$rebate->ordernum = $data['ordernum'];
    		$rebate->save();
    		if($rebate->save()){
    			return json_encode([
    					'errorcode'=>0,
