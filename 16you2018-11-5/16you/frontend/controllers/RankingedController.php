 ...
 * @author 
 */
class RankingedController extends BaseController{
	...
	public function actionTurnplateluck(){
		...
		if($c_num>0){
			...
			if($ran<10){//红包
				$price = ($info==6)?168:(($info==8)?20:1.68);
				$win->content = '红包'.$price.'元';
				$wx = new Wxpayutil();
				$openid = yii::$app->session['rpopenid'];
				$partner_trade_no = 'wxp'.date('YmdHis').rand(1000,9999);
				$p_res = $wx->sendredpacket($openid,$partner_trade_no,$price,''); //发红包
				if($p_res){
					$win->type = 2;//红包发送成功
				}else{
					$win->type = 3;//红包发送失败
