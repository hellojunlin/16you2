	                     ...
	                     <?php if(YII::$app->session['role']!='-1'):?> 
			               <?php foreach (yii::$app->session['mdata'] as $mdata):?>
					          <?php if($mdata['child']=='game/toadd'):?>
		                        <button class="btn btn-primary" onclick="window.location.href='<?=Url::to(['toadd'])?>'"><i class="fa  fa-plus"></i> 添加记录 </button>&nbsp;
		                  	  <?php endif;?>
						    <?php endforeach;?>
						   <?php else :?>
					            <button class="btn btn-primary" onclick="window.location.href='<?=Url::to(['toadd'])?>'"><i class="fa  fa-plus"></i> 添加记录</button>&nbsp;
	                    	...
	                    	<div class="form-group">
	                    		<form action="">
                                    ...
                                    <div class="col-sm-8" style="padding:0px 1px">
                                        <input type="text" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="请输入游戏名称" data-original-title="请输入游戏名称" name="keyword" maxlength="50" value="<?php echo $value;?>">
                                    </div>
                                  </form>
	                        </div>
	                    </div>
	                    ...
	                    <table class="table table-bordered table-striped table-condensed cf">
	                        ...
	                        <?php if($data['data']): ?>
								...
								<tr>
		                            ...
		                            <td><?= Html::encode($v['service_code'])?></td>
		                            <td><?php echo  date('Y-m-d H:i',$v['open_time'])?></td>
		                            <?php if($v['state']==0):?>
		                            ...
		                            <td>
				                            ...
				                            <?php endif;?>
				                            <button class="btn btn-info btn-xs" onclick="window.location.href='/service/toedit/<?php echo $v['id']; ?>.html'"><i class="fa fa-edit"></i>&nbsp;编辑</button>
			                                <button class="btn btn-danger btn-xs del" id="<?php echo $v['id'];?>"><i class="fa fa-trash-o"></i>&nbsp;删  除</button>
			                                <button class="btn btn-success btn-xs" onclick="window.location.href='/service/tocopyadd/<?php echo $v['id']; ?>.html'"><i class="fa fa-edit"></i>&nbsp;添加</button>
			                        </td>
		                        </tr>
	                        	<?php endforeach;?>
