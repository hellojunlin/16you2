                ...
                <div class="panel red-bg">
                    <div class="panel-body">
                        <div class="row">
                            ...
                            <div class="col-xs-8">
                                <span class="state-title"> 已返利 </span>
                                <h4>¥ <?php echo isset($allrebatearr[0]['num'])?$allrebatearr[0]['num']:0;?>元</h4>
                            </div>
                        </div>
                    </div>
                </div>
	                    ...
	                    <table class="table table-bordered table-striped table-condensed cf">
	                        ...
	                        <?php if($data['data']): ?>
								...
								<tr>
		                            ...
		                            <?php endif;?>
		                             <td><?php if( mb_strlen($v['remark'],"UTF8")<12): echo ($v['remark']=='-1')?'小程序转盘提现':$v['remark'];?>
		                                <?php else:?>
		                            ...
		                            <td><?php echo empty($v['passtime'])?'':date('Y-m-d H:i:s',$v['passtime']); ?></td>
		                            <td>
		                           		<?php if($v['state']==0):?>
		                                <button class="btn btn-info btn-xs passbtn" id="<?= Html::encode($v['id'])?>" name="<?= Html::encode($v['uid'])?>" type="<?php echo ($v['remark']=='-1')?'1':'';?>"><i class="fa fa-edit"></i>&nbsp;通过</button>
...
<script>
	...
	$('body').on('click','.passbtn',function(){
		...
		var uid = _this.attr('name');
		var type = _this.attr('type');
		if(confirm('确认通过审核吗？')){
			$.ajax({
		        url:"/rebate/passrebate.html",
		        type:'post',
		        dataType:'json',
		        data:{
			        'id':id,
