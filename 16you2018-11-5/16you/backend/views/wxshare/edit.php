...
<section class="wrapper">
  <div class="row">
    <div class="col-lg-12">
            <section class="panel">
                <header class="panel-heading">编辑微信分享</header>
                <div class="panel-body">
                    <form class="form-horizontal adminex-form" method="post" id="signupok" enctype="multipart/form-data">
												...
												<ul class="list-unstyled">
												 <?php if($game): ?>
												  <?php foreach ($game as $v):?>
                                						<?php if(isset($v['id'])): ?>
													<li class="filter-item items" data-value="<?php echo $v['id'].'%@!'.$v['name'];?>" <?php if($v['id'].'%@!'.$v['name']==$model->gid.'%@!'.$model->gamename){echo 'id="selectedgame"';}?>><?php echo $v['name'];?></li>
													 <?php endif; ?>
                               					 <?php endforeach;?>
                               					 <?php endif; ?>
												</ul>
                       ...
                       <!-- <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control m-bot15" name="gid" id="gid">
                                    <?php foreach ($game as $v) {
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="标题"  name="title" maxlength="25" value="<?php echo $model->title; ?>" required />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 col-sm-2 control-label">描述</label>
                            <div class="col-sm-5">
                                <textarea rows="4" class="form-control" name="desc"><?php echo $model->desc; ?></textarea>
                            </div>
                        </div>
                       <div class="form-group">
                            <label class="col-sm-2 col-sm-2 control-label">跳转的链接</label>
                            <div class="col-sm-5">
                                <input type="url" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="跳转的链接"  name="link"  value="<?php echo $model->link;?>"/>
                        ...
                        <div class="form-group">
                            <label class="col-sm-2 col-sm-2 control-label">分享图标</label>
                             <div class="upload_btn col-sm-10">
                                 <div class="up_img" style="<?php //echo $model->imgUrl?'display:none':'';?> ">
                                    <img src="/media/images/putimg.png">
                                    <input class="input_upimg" type="file" id="upload_image" name="myFile" value="图片上传"
                                         capture="camera">
                                 </div>
                                 <?php //if($model->imgUrl): ?>
                                 <div class="p_img delimg">
                                     <i class="fa fa-times-circle fa-2x img_circle"></i>
                                     <img src="/media/images/wxshare/<?php //echo $model->imgUrl;?>">
                                     <input type="hidden" id="file" name="image" value="<?php //echo $model->imgUrl;?>">
                                 </div>
                                 <?php //endif; ?>
                             </div>
                        </div>
...
var name_id = "<?php echo $model->gid.'%@!'.$model->gamename; ?>";
$("#gid option[value='"+name_id+"']").attr('selected',true);
//异步提交数据
