            ...
            <section class="panel">
                <div class="panel-body">
                    <form class="form-horizontal adminex-form" method="post" id='signupok' enctype="multipart/form-data">
                       <input type="hidden" name="id" value='<?php echo $sendmsg->id;?>'>
                        <div class="form-group">
                            <label class="col-sm-2 col-sm-2 control-label">模板类型</label>
                            <div class="col-sm-5">
                            <input type="hidden" name="temp" value="<?php echo $sendmsg->t_id;?>">
                                <select class="form-control m-bot15" name="tempaaa" id="temp" disabled="disabled">
                                 <?php $sendtmp = yii::$app->params['sendTmp']; foreach ($sendtmp as $k=>$temp):?>
                                    <option value='<?php echo $k;?>' <?php if($k==$sendmsg->t_id) echo 'selected';?>><?php echo $temp;?></option>
                                 <?php endforeach;?>
                                </select> 
                            </div>
                        </div>
                         <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="标题" data-original-title="标题" name="title" maxlength="16" value="<?php echo $sendmsg->title;?>"/><span></span>
                            </div>
                        </div>
                         <div class="form-group">
                            <label class="col-sm-2 control-label">跳转链接</label><span style="color:#e00">（不填则不跳转）</span> 
                            <div class="col-sm-5">
                                <input type="text" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="链接" data-original-title="链接" name="url" maxlength="255" value="<?php echo $sendmsg->url;?>"/><span></span>
                            </div>
                       </div>
...
<script>
serdarr =<?php echo $sendmsg->data;?>;
console.log(serdarr);
$(document).ready(function(){
	 var selected = $('#temp').val();
	 if(selected=='Xh9z-YEgvGauhwWxgZrdKAA9ET-ow6PZdEv39bdcKgk'){//下单成功通知
		 ...
		 $.each(dataarr, function(k,v){  
					 ...
					 $('<input type="text" name="keyword[]" maxlength="100 required" value="'+serdarr[k]['v']+'">').attr('placeholder',v).addClass('form-control tooltips').appendTo(div);
					 $('<span>').appendTo(div);
					 $('<input>').addClass('select-color btn btn-primary').attr({'id':'AlterColors'+index,'value':serdarr[k]['c'],'type':'text'}).appendTo(form);
					 var hiddeninput = $('<input>').attr({'type':'text','class':'hideinput AlterColors'+index+'','value':serdarr[k]['c'],'name':'color[]'}).appendTo(form);
		 ...
		 $.each(dataarr, function(k,v){  
				 ...
				 var div = 'div'+index;
				 var div= $('<div>').addClass('col-sm-5').appendTo(form);
				 $('<input type="text" name="keyword[]" maxlength="100" value="'+serdarr[k]['v']+'">').attr('placeholder',v).addClass('form-control tooltips').appendTo(div);
				 $('<span>').appendTo(div);
				 $('<input>').addClass('select-color btn btn-primary').attr({'id':'AlterColors'+index,'value':serdarr[k]['c'],'type':'text'}).appendTo(form);
				 var hiddeninput = $('<input>').attr({'type':'text','class':'hideinput AlterColors'+index+'','value':serdarr[k]['c'],'name':'color[]'}).appendTo(form);
				 $.fn.jPicker.defaults.images.clientPath='/media/images/selectcolor/';
