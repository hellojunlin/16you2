                    ...
                    <div class="clearfix">
                     <?php if(YII::$app->session['role']!='-1'):?> 
                        <?php foreach (yii::$app->session['mdata'] as $mdata):?>
                          <?php if($mdata['child']=='autoreply/add'):?>
                            <button class="btn btn-primary" onclick="window.location.href='<?=Url::to(['add'])?>'"><i class="fa  fa-plus"></i> 添加数据 </button>&nbsp;
                          <?php endif;?>
                        <?php endforeach;?>
                     <?php else:?>
                            <button class="btn btn-primary" onclick="window.location.href='<?=Url::to(['add'])?>'"><i class="fa  fa-plus"></i> 添加数据 </button>&nbsp;
                            ...
                            <div class="form-group">
                                <form action="<?=Url::to(['index'])?>">
                                    ...
                                    <div class="col-sm-9" style="padding:0px 1px">
                                        <input type="text" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" title="" placeholder="请输入内容..." data-original-title="请输入内容" name="value" maxlength="16" value='<?php echo $value; ?>'/>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <table class="table table-bordered table-striped table-condensed cf">
                            ...
                            <?php if($data['data']): ?>
                                ...
                                <tr>
                                    ...
                                    <td><?=Html::encode($v['state']==1?'启用':'禁用')?></td>
                                    <td><?= Html::encode(date('Y-m-d',$v['createtime'])) ?></td>
                                    <td>
                                     <?php if(YII::$app->session['role']!='-1'):?> 
                                       <?php foreach (yii::$app->session['mdata'] as $mdata):?>
                                           <?php if($mdata['child']=='autoreply/edit'):?>
                                            <button class="btn btn-info btn-xs" onclick="window.location.href='/autoreply/edit/<?php echo $v['id']; ?>.html'"><i class="fa fa-edit"></i> 编辑</button>
                                             <?php endif;?>
                                             <?php if($mdata['child']=='autoreply/del'):?> 
...
<script>
    //删除
    function del(id){
        layer.msg('正在删除中。。。',{icon:1,time:1000});
            $.ajax({ 
                url:"/autoreply/del.html",
                type:'post',
                data:{'id':id},
                success:function(data){
                    if(data==1){
                        layer.msg('删除成功',{icon:6,time:1000});
                        setInterval(location.href='/autoreply/index.html',900);
                    }else{
                        alert('删除失败');
                    }
                }
