...
class CountController extends BaseController{
	public function actionIndex(){
		...
		if($manage_pid){//平台管理则或者平台商
			...
			if(yii::$app->session->get('platepid')==6){
            	$setime = 1501516800;//2017 08 01 00:00:00
            	$isboolen = true;
            }
		}else{//超级管理员
			$p_where = '';
		...
		if($curPage==1){//从数据库读取今天的记录
			if(($starttime == $date || $endtime == $date)||($starttime==strtotime(date('1970-01-01'))&& $endtime==time())){ //查询日期属于今天
				//付费用户数
				$gameorder = \Yii::$app->db->createCommand("SELECT T.createtime as count_time,T.gid,T.gamename, COUNT(*) AS pay_user,SUM(S_price) as pay_sum,T.date,COUNT(pay_num) as pay_num FROM (SELECT curdate() AS date,SUM(price) S_price,O.createtime,O.uid, O.gid,COUNT(O.id) as pay_num, G.name AS gamename FROM g_game G LEFT JOIN g_order O ON G.id = O.gid WHERE O.state = 2 and FROM_UNIXTIME(O.createtime,'%Y-%m-%d') = curdate() $pgwhere $where $mhide and O.pid in($pidstr) GROUP BY date, O.uid, O.gid ) T GROUP BY T.Date, T.gamename")->queryAll();
				if($gameorder){
					$gameorder1 = \Yii::$app->db->createCommand("SELECT curdate() AS date,COUNT(*) cuser,SUM(O.price) cprice,O.gid FROM g_order as O where O.utype=2 and O.state=2 and O.pid in($pidstr) $pgwhere $mhide and FROM_UNIXTIME(O.createtime,'%Y-%m-%d') = curdate() group By O.gid,O.uid")->queryAll();//查出每天新增用户数和新增金额总数
		...
		if($newarrcount>=$pageSize){ //数据数大于显示数 则无需查数据库
			...
			->select('FROM_UNIXTIME(count_time) as date ,gamename,count_time,sum(new_user) as new_user,sum(play_user) as play_user,sum(pay_user) as pay_user ,sum(pay_probability) as pay_probability ,sum(pay_sum) as pay_sum,sum(cuser) as cuser,sum(cprice) as cprice,sum(old_user) as old_user,sum(pay_num) as pay_num,gid')
			->from('g_count')
			...
			$setime && $countarr = $countarr->andWhere(['>=','count_time',$setime]);
		...
		yii::$app->session['localsecondurl'] = yii::$app->params['backend'].'/count/index.html';
		...
		return $this->render('detailcount',[
				'data'=>$data,
				'starttime'=>$startdate,
				'endtime'=>$enddate,
				'pages' => $pages,
	...
	public function actionTocount(){
		...
		if($manage_pid){//平台管理则或者平台商
			$p_where = ['id'=>$manage_pid];
			if(yii::$app->session->get('platepid')==6){//招河规定该平台只显示2017年8月份数据，有问题询问他，
            	$setime = 1501516800;//2017 08 01 00:00:00
		...
		$starttime = $start_date?strtotime($start_date):strtotime(date('Y-m-01'));
		$endtime = $end_date?strtotime($end_date)+86399:time();
		$curPage = Yii:: $app->request->get( 'page',1);
    	$pageSize = 200;
    	$pidwherearr = '';
		//数据库统计表的数据
		$query = (new \yii\db\Query())
		->select('FROM_UNIXTIME(count_time),count_time,sum(new_user) as new_user,sum(play_user) as play_user,sum(pay_user) as pay_user ,sum(pay_probability) as pay_probability ,sum(pay_sum) as pay_sum,sum(cuser) as cuser,sum(cprice) as cprice,sum(old_user) as old_user,sum(pay_num) as pay_num')
		->from('g_count')
		->where("count_time between $starttime and $endtime and pid in ($pidstr)")
		->groupBy('count_time')
		->orderBy('count_time desc');
		$pid && $query = $query->andWhere(['pid'=>$pid]);
		$setime && $query = $query->andWhere(['>=','count_time',$setime]);
		...
		if($curPage==1){
			if($starttime <= $date && $endtime > $date){ //查询日期不属于今天
				...
				//付费率和充值流水
				$gameorder = \Yii::$app->db->createCommand("SELECT count(distinct uid) as pay_user,count(id) AS pay_num,SUM(price) AS pay_sum,createtime as count_time,(SELECT count(distinct uid) FROM g_order WHERE utype=2 and state=2 $mhide and createtime>=:time1 $wpid and pid in ($pidstr)) AS cuser,(SELECT sum(price) FROM g_order WHERE utype=2 and state=2 $mhide and createtime>=:time1 $wpid and pid in ($pidstr)) AS cprice FROM g_order WHERE state = 2 $mhide AND createtime>=:time1 $wpid and pid in ($pidstr)",[':time1'=>$date])->queryOne();
		...
		//菜单定位
		unset(yii::$app->session['localfirsturl']);
		yii::$app->session['localsecondurl'] = yii::$app->params['backend'].'/count/tocount.html';
		
		$hidearr = '';
		if($manage_pid){
			$hidequery = (new \yii\db\Query())
						 ->select('pid,price,createtime,gid')
						 ->from('g_order_hide')
						 ->where("createtime between $starttime and $endtime and pid in ($pidstr)")
						 ->groupBy('createtime')
						 ->orderBy('createtime desc')
						 ->all();
			if($hidequery){//存在时
				foreach ($hidequery as $h){
					 isset($hidearr[$h['createtime']])? $hidearr[$h['createtime']] +=$h['price'] : 0 ;
				}
			}
		}
		$ppid = $pid?"and pid={$pid}":'';
		//游戏总览
		$zcount = \Yii::$app->db->createCommand("SELECT sum(price) zprice,count(distinct uid) zuid from g_order where state=2  $hide_m $gcreatime1 $gp_where1  and createtime between $starttime and $endtime $ppid")->queryOne();//总付费用户数和总付费金额(元)
		
		return $this->render('collectcount',[
				'data'=>$data,
    ...
    public function actionOutput(){
        ...
        if($starttime <= $date && $endtime > $date){ //查询日期不属于今天
			...
			//付费率和充值流水
			$gameorder = \Yii::$app->db->createCommand("SELECT count(distinct uid) as pay_user,count(id) AS pay_num,SUM(price) AS pay_sum,createtime as count_time,(SELECT count(distinct uid) FROM g_order WHERE utype=2 and state=2 and createtime>=:time1 $wpid) AS cuser,(SELECT sum(price) FROM g_order WHERE utype=2 and state=2 and createtime>=:time1 $wpid) AS cprice FROM g_order WHERE state = 2 AND createtime>=:time1 $wpid",[':time1'=>$date])->queryOne();
			if($gameorder){
				if(!$gameorder['cprice']){$gameorder['cprice']='0.00';}
				$gameorder['play_user'] = $playusernum;
				$gameorder['new_user'] = $newusernum;
		...
		->select('count_time,sum(new_user) as new_user,sum(play_user) as play_user,sum(pay_user) as pay_user ,sum(pay_sum) as pay_sum,sum(cuser) as cuser,sum(cprice) as cprice,sum(old_user) as old_user,sum(pay_num) as pay_num')
		->from('g_count')
        ...
        $header = ['编号','日期','激活数','新增用户数','老活跃用户数','总付费人数','总付费金额(元)','新增付费人数','新增付款金额(元)','付费次数','ARPU(元)','ARPPU(元)','付费率(%)'];
        $arr = array();
        if(isset($gameorder)&&$gameorder){
        ...
        foreach ($data as $k => $v) {
            ...
            $arr[$k]['new_user'] = $v['new_user'];
            $arr[$k]['old_user'] = $v['old_user'];
            $arr[$k]['pay_user'] = $v['pay_user'];
            $arr[$k]['pay_sum'] = $v['pay_sum'];
            $arr[$k]['cuser'] = $v['cuser'];
            $arr[$k]['cprice'] = $v['cprice'];
        ...
        Phpexcelr::exportData($arr,$header,$time."汇总导出",$time."汇总导出");
    ...
    public function actionDetailsoutput(){
		...
		if($data){
			foreach ($data as $key => $value) {
				$data[$key]['ARPU'] = ($value['play_user'])?round($value['pay_sum']/$value['play_user'],2):'';
				$data[$key]['ARPPU'] = ($value['pay_user'])?round($value['pay_sum']/$value['pay_user'],2):'';
				$data[$key]['pay_probability'] = ($value['play_user'])?round($value['pay_user']/$value['play_user'],2):'';
			}
		}
		//查询之前的统计
		if($newarr){//如果今天的记录不为空 则合并
			foreach ($newarr as $na){
				$na['ARPU'] = ($na['play_user'])?round($na['pay_sum']/$na['play_user'],2):'';
				$na['ARPPU'] = ($na['pay_user'])?round($na['pay_sum']/$na['pay_user'],2):'';
				$na['pay_probability'] = ($na['play_user'])?round($na['pay_user']/$na['play_user'],2):'';
        ...
        $header = ['编号','日期','游戏名称','日DAU','新增用户数','老活跃用户数','付费人数','总付费金额(元)','新增付费人数','新增付款金额(元)','付费次数','ARPU(元),ARPPU(元),付费率(%)'];
        foreach ($data as $k => $v) {
            $arr[$k]['v'] = $k+1;
			$arr[$k]['count_time'] = date('Y-m-d',$v['count_time']);
       	...
       	$time = $start_time?($start_time.' - '.$end_time.'_'):'';
        Phpexcelr::exportData($arr,$header,$time."详细统计导出",$time."详细统计导出");
        exit;//阻止跳转，一定要写，不写会跳转
    }
    ...
    public function actionMonthcount(){
		...
		if($setime){
			$time_s = "and createtime>=$setime";
			$time_ss = "and createtime between $start_time and $end_time";
		}else{
			$time_s = '';
			$time_ss = '';
		}
		//游戏总览
        $zcount = \Yii::$app->db->createCommand("SELECT sum(price) zprice,count(distinct uid) zuid from g_order where state=2 $mhide $gcreatime1 $wpid $gp_where1 ")->queryOne();//总付费用户数和总付费金额(元)

        $cplay = Playgameuser::find();
		$manage_pid && $cplay = $cplay->where(['pid'=>$manage_pid]);
		$setime && $cplay = $cplay->andWhere(['>=','createtime',$setime]);
		$cplay = $cplay->groupBy('uid')->count();
    	$pages = new Pagination([ 'totalCount'=>$data[ 'count'], 'pageSize' => $pageSize]);
    	//菜单定位
    	unset(yii::$app->session['localfirsturl']);
    	yii::$app->session['localsecondurl'] = yii::$app->params['backend'].'/count/monthcount.html';
    	return $this->render('monthcount',[
    			'data'=>$data,
    			'starttime'=>$starttime,
    			'endtime'=>$endtime,
    			'pages'=>$pages,
    ...
    public function actionMdataexport(){
    	...
    	if($curPage==1 && ($starttime == $month || $starttime=='')){//当月的数据
    		...
    		//付费率和充值流水
    		$gameorder = \Yii::$app->db->createCommand("SELECT count(distinct uid) as pay_user,count(id) AS pay_num,SUM(price) AS pay_sum,createtime as count_time,(SELECT count(distinct uid) FROM g_order WHERE utype=2 and state=2 and createtime between $monthfirst and $monthlast $wpid) AS cuser,(SELECT sum(price) FROM g_order WHERE utype=2 and state=2 and createtime between $monthfirst and $monthlast $wpid) AS cprice FROM g_order WHERE state = 2 and createtime between $monthfirst and $monthlast $wpid and pid in ($pidstr)")->queryOne();
    		if($gameorder&&$gameorder['count_time']){//判断是否有数据
    			if(!$gameorder['cprice']){$gameorder['cprice']='0.00';}
    			$gameorder['play_user'] = $playusernum;
    			$gameorder['new_user'] = $newusernum;
    	...
    	$header = ['编号','日期','月DAU','新增用户数','老活跃用户数','付费人数','总付费金额(元)','新增付费人数','新增付费金额','付费次数','ARPU(元)','ARPPU(元)','付费率'];
    	foreach ($data['data'] as $k => $v) {
    		$arr[$k]['v'] = $k+1;
    		$arr[$k]['count_time'] = isset($v['count_time'])?date('Y-m',$v['count_time']):0;
    		$arr[$k]['play_user'] = isset($v['play_user'])?$v['play_user']:0;
    		$arr[$k]['new_user'] = isset($v['new_user'])?$v['new_user']:0;
    		$arr[$k]['old_user'] = isset($v['old_user'])?$v['old_user']:0;
    		$arr[$k]['pay_user'] =  isset($v['pay_user'])?$v['pay_user']:0;
    		$arr[$k]['pay_sum'] =  isset($v['pay_sum'])?$v['pay_sum']:0;
    		$arr[$k]['cuser'] =  isset($v['cuser'])?$v['cuser']:0;
    		$arr[$k]['cprice'] =  isset($v['cprice'])?$v['cprice']:0;
    	...
    	Phpexcelr::exportData($arr,$header,"月汇总导出","月统计导出");
    	exit;//阻止跳转，一定要写，不写会跳转
    ...
    public function actionCountfoleline() {
        ...
        $setime && $count = $count->andWhere("count_time>=$setime") ;
        $re_data = array();
