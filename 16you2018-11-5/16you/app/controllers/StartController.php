...
class StartController extends Controller{
    ...
    public function actionIndex() { 
    	...
    	$logintype=1;//默认会员登陆
    	if(isset(yii::$app->session['user'])||!empty(yii::$app->session['user'])){//登陆后的状态
    		//return $this->redirect('/personal/index.html');
	    	//获取用户信息
	    	$user = yii::$app->session['user'];
	    	//获取游戏信息
	    	$this->changesession($game); //更新最近在玩缓存
	    	$acctoken = $user->access_token.'|'.$user->id.rand(1000,9999); //获取access_token
    	}else{
    		$logintype = 2;//游客登陆
    		$acctoken = isset(yii::$app->session['access_token'])?yii::$app->session['access_token']:''; //'test5102216you666';//游客accesstoken
    	}
     	$res = strstr($game['game_url'],"?");
        if($res){
        	$url = $game['game_url'].'&access_token='.$acctoken;  //游戏链接带参数
        }else{
            $url = $game['game_url'].'?access_token='.$acctoken;  //游戏链接带参数
        }
        $gname = $game['name'];
        //获取广告图片
        // $plate = PlatformRedis::find()->where(['punid'=>$puid])->one();
        // if(!$plate){
            $plate = Plateform::find()->where(['punid'=>$puid])->one();
        //     $plate && $this->toPlate($plate);
        // }
        if($plate){
    ...
    */
    ...
    public function actionSaveplayuser(){
    	if(yii::$app->request->isAjax && isset($_POST['gid']) && isset($_POST['usertype'])){
    		$gid = Helper::filtdata($_POST['gid'],'INT');  //游戏ID
    		$usertype = Helper::filtdata($_POST['usertype'],'INT');  //用户类型   1：会员  2：游客
    		if(!$gid || !$usertype){
    			return json_encode([
    					'errorcode'=>1001,
    					'info'=>'网络异常，稍后再试',
    		...
    		if($usertype==1){//会员登录
    			...
    			$userarr = array();
    			$userarr['id'] = yii::$app->session['touristid'];  //用户id
    			$userarr['pid'] = 14;  //app不区分平台  默认为16游平台         平台id
    		}
    		
    		$saveres = $this->saveplayuser($userarr,$gid,$usertype);//保存当天玩游戏用户记录和最近在玩记录
    		if(!$saveres){
    			return json_encode([
    					'errorcode'=>1001,
    					'info'=>'网络异常，稍后再试',
   ...
   /**
     ...
     * $usertype 1:会员   2:游客
     */
    private function saveplayuser($userarr,$gid,$usertype=1){
    	...
    	if($usertype==1){ //会员登录
    		...
    		$uid = $user->id;
    		$pid = $user->pid;
    		$playuserres = Playgameuser::find()->where(['uid'=>$uid])->andWhere("createtime>$sixtime")->all();//查找该用户的游戏记录
    	}else{
    		$uid = $userarr['id'];
    		$pid = $userarr['pid'];
    		$playuserres = Touristplaygameuser::find()->where(['uid'=>$uid])->andWhere("createtime>$sixtime")->all();//查找该用户的游戏记录
    	}
    	...
    	if($isboolean){//未有记录，则保存数据库 反之，否则不做处理
    		...
    		if($usertype==1){//会员登录
    			...
    			$playgameuser = new Touristplaygameuser();
    			$playgameuser->client_type = 2;
    		}
    	 	$playgameuser->uid = $uid;
    		$playgameuser->gid = $gid;
    		$playgameuser->pid = $pid;
    ...
    public function actionTouristlogin(){
    	...
    	$ip = '19'.substr(str_replace('.', '', $_SERVER["REMOTE_ADDR"]),-5,6).rand(100,999);
    	yii::$app->session['touristid'] = $ip;
    	yii::$app->session['access_token']='test2018116you666|'.$ip;//游客accesstoken
    	return json_encode([
    			'errorcode'=>0,
    			]);
    }
