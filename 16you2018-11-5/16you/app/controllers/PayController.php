...
class PayController extends BaseController{
	 ...
	 * sign : 请求参数签名
	 * ptype : 支付类型   1:微信支付  2：盛付通微信支付 3：盛付通支付宝支付 4：盛付通网银支付  5：盛付通H5快捷支付  6：微信扫码支付  7: 盛付通微信扫码支付 8：游币支付 9：优赋微信H5支付    10:优赋扫码支付  
	 * payurl : 支付请求链接
	 */
	public function actionAllsftpay(){
		if(Yii::$app->request->isAjax){
			...
			if(($out_trade_no||$product_id)==false ||$out_trade_no===false||$product_id===false || $total_fee===false ||$sign===false  || $ptype===false ||$payurl===false){
			...
			if($order->save()){
				...
				}elseif($ptype==11){//11:优赋支付宝支付
					$requesturl = $this->unhpay($order, $payurl,'42');
				}elseif($ptype==12){//11:优赋app支付宝支付
