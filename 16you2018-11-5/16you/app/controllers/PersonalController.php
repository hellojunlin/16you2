...
class PersonalController extends Controller{
    ...
    public function actionCode(){
        ...
        $filename = $wxutil->gettempcode($appid,$verify,dirname(dirname(__FILE__)).'/web/media/images/code/');
        if($filename){
            return json_encode([
                'info'=>$filename,
                'verify'=>$verify,
 ...
 /**
    ...
    public function actionInformation(){
        ...
        if(yii::$app->session['playgame']){ 
            ...
            $playgame = yii::$app->session['playgame']?array_slice(yii::$app->session['playgame'],0,5):array();
        }
        //获取游戏信息
